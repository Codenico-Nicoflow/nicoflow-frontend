...
`morningHour` (5–11, default 8) is the local hour the morning sweeps fire at (day-start, inbox, overdue, due-notify); `eveningHour` (18–22, default 20) drives the end-of-day summary. Both are range-validated (out of range → `INVALID_INPUT`) and backed by a DB CHECK. Each sweep fires within a 3-hour catch-up window from its hour (insures against a missed hourly tick — DST, a failed cron run, a cold start), clamped so it never wraps past midnight.
