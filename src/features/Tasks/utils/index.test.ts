...
describe('formatTaskGentleDate', () => {
  it('returns null when the task is unscheduled', () => {
    expect(formatTaskGentleDate({ scheduledFor: null })).toBeNull();
  });

  it('a past soft scheduledFor reads as "carried over" — never red', () => {
    const result = formatTaskGentleDate({ scheduledFor: isoDate(subDays(new Date(), 3)) });
  ...
  it('a soft scheduledFor today/tomorrow uses the soft (non-red) tone', () => {
    const today = formatTaskGentleDate({ scheduledFor: isoDate(new Date()) });
    const tomorrow = formatTaskGentleDate({ scheduledFor: isoDate(addDays(new Date(), 1)) });
  ...
  it('a future soft scheduledFor formats the date, neutral tone', () => {
    const result = formatTaskGentleDate({ scheduledFor: isoDate(addDays(new Date(), 5)) });
